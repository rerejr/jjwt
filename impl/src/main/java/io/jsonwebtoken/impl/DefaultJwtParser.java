...
public class DefaultJwtParser implements JwtParser {
    ...
    public Jwt<?,?> parse(String compact) throws ExpiredJwtException, MalformedJwtException, SignatureException {
        ...
        if (tokenized instanceof TokenizedJwe) {
            ...
            if (!Strings.hasText(tokenized.getDigest()) && SignatureAlgorithms.NONE.getName().equalsIgnoreCase(alg)) {
        ...
        if (jwt instanceof Jws) { // it's a JWS, validate the signature
            ...
            if (key != null && signingKeyResolver != null) {
                throw new IllegalStateException("A key and a signing key resolver cannot both be specified. Choose either.");
            }

            //digitally signed, let's assert the signature:
            Key key = this.key;

            if (key == null && signingKeyResolver != null) { //fall back to signing key resolver
                if (claims != null) {
